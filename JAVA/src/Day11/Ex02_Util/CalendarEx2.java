...
//		지정된 형식으로 변환하는 SimpleDateFormat
...
//				이번 주 로또 추첨 일시 구하기!
//				매일 6시부터 24시까지 판매
//				추첨일(토요일)에는 오후 8시까지 판매 마감
//				추첨일 오후 8시부터 다음날(일요일) 오전 6시까지는 판매 정지
				
//				조건
//				일요일 오전 6시 ~ 토요일 8시 사이에 구매 가능 : 이번주 토요일 오후 9시가 추첨하는 시간
				
//				=> 로또구매를 조건에 따라 제한하고, 로또를 구매할 경우 추첨일이 나오는 기능을 구현해보기
...
//				3. 일요일이면서, 오전 6시 이후 - 다음 주 토요일 9시가 추첨일시
//				4. 그 외, 이번 주 토요일 9시가 추첨일시 [+(7-요일)]				//<-일요일을 1로 가정했을 때, 무조건적으로 6번째 다다르는 룰.
...
//				1. 토요일이면서, 오후 8시 이후 - 구입불가
				if(todayOfWeek.equals("토")&& ampm ==1 && 시간 >=8) {
					System.out.println("구입불가");
				}
//				2.일요일이면서, 오전 6시 이전 - 구입불가
