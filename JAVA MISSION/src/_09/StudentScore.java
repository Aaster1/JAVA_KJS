...
public class StudentScore {
			...
			if (classYn[i]) {
				...
				for (int j = 0; j < classStudentScore[i].length; j++) {
					if (classYn[i] && studentYn[i][j]) {
						...
						System.out
								.print((classStudentRank[i][j][0] + 1) + "\t" + (classStudentRank[i][j][1] + 1) + "\t");
						System.out.println();
					}
				}
				System.out.println(
	...
	public static void classList() {
		...
		for (int i = 0; i < classStudentScore[classNo - 1].length; i++) {
			if (classYn[classNo - 1] && studentYn[classNo - 1][i]) {
				...
				System.out.print((classStudentRank[classNo - 1][i][0] + 1) + "\t"
		...
		for (int i = 0; i < classStudentScore.length; i++) {
			...
			for (int j = 0; j < classStudentScore[i].length; j++) {
				if (classYn[i] && studentYn[i][j]) {
					...
					System.out.print((subjectStudentRank[i][j][subjectNo - 1][0] + 1) + "\t");
					System.out.print((subjectStudentRank[i][j][subjectNo - 1][1] + 1) + "\t");
					System.out.println();
				}
			}
			if (classYn[i])
	...
	public static void subjectList(int subjectNo) {
		...
		for (int j = 0; j < classStudentScore[classNo - 1].length; j++) {
			if (classYn[classNo - 1] && studentYn[classNo - 1][j]) {
				...
				System.out.print((subjectStudentRank[classNo - 1][j][subjectNo - 1][0] + 1) + "\t");
				System.out.print((subjectStudentRank[classNo - 1][j][subjectNo - 1][1] + 1) + "\t");
				System.out.println();
			}
		}
		if (classYn[classNo - 1])
	...
	public static void searchStudent() {
		...
		if (classYn[classNo - 1] && studentYn[classNo - 1][studentNo - 1]) {
			...
			System.out.print((classStudentRank[classNo - 1][studentNo - 1][0] + 1) + "\t"
					+ (classStudentRank[classNo - 1][studentNo - 1][1] + 1) + "\t");
			System.out.println();
		}
	...
	public static void calcRank(int classNo) {
		...
		for (int i = 0; i < classStudentData[classNo-1].length; i++) {
			...
			classStudentRank[classNo-1][i][0]=rank;
		}
		// 반 등수 계산 (끝)
		...
		for (int i = 0; i < classStudentScore[classNo-1].length; i++) {
			for (int j = 0; j < classStudentScore[classNo-1][i].length; j++) {
				int rank=0;
				for(int ii = 0; ii <classStudentScore[classNo-1].length; ii++) {
				if(classStudentScore[classNo-1][i][j]<classStudentScore[classNo-1][ii][j]) {
							rank++;
				}
			}
				subjectStudentRank[classNo-1][i][j][0]=rank;
				}
		}
		// 과목별 반 등수 계산 (끝)
...
//		인덱스 에러. 10을 넘아가는 i or j , k , l 이있음
				...
				for (int k = 0; k < classStudentScore[i][j].length; k++) {
					...
					for (int l = 0; l < classStudentScore.length; l++) {
						for (int m = 0; m < classStudentScore[l].length; m++) {
							if(classStudentScore[i][j][k]<classStudentScore[l][m][k]) {
										rank++;
							}
					}
				}
					subjectStudentRank[i][j][k][1]=rank;
			}
		}
		}
	}
	...
	public static void calcClassTotal(int classNo) {
		...
		for (int i = 0; i < classStudentScore[classNo - 1].length; i++) {
			for (int j = 0; j < classStudentScore[classNo - 1][i].length; j++) {
				classTotalAverage[classNo - 1][j] = Math
						.round((double) classTotalScore[classNo - 1][j] / classCount[classNo - 1] * 100.0) / 100.0;
			}
		}
